...
class GameViewModel : ViewModel() {
    ...
    private fun getNextWord(){
        ...
        val wordToCharArray = currentWord.toCharArray()

        while(String(wordToCharArray).equals(currentWord, ignoreCase = true)){
            wordToCharArray.shuffle()
        }
        if(usedWords.contains(currentWord)){
            getNextWord()
        }else{
            _currentScrambledWord.value = String(wordToCharArray)
